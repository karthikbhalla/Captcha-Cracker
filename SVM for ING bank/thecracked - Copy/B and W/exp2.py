...
for r in range(1,2):
	...
	im = Image.open("output"+str(xcount)+".gif")
	im = im.convert("L")
...
#print pix
	...
	for x in range(0,im3.size[0]):
		...
		if(countb>=2 and flag2==0): #to strt the crop line
			startx=tx
			starty=ty
			flag2=1
			im4.putpixel((startx,starty),200) # not important
			
		if(countb<2 and flag2==1): #to find the end point of the crop part
			#print 'hi'
			endx=x-startx
			...
			working_slice.save("outputcrop"+str(ccount)+".gif",quality=95) #to save the crop pik
			if(countt<3): # if total count (mentiond above) is small , then delete the crop (for small dot disturbances)
				os.remove("outputcrop"+str(ccount)+".gif")
			else:
				destinationDir='C:\Users\Prasanth\Desktop\\comb\\Captcha-Cracker-master\\thecracked - Copy\\B and W\\org' # to save the crops into a folder
