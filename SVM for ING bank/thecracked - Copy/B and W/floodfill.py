...
checky=0			
scount=0
for abc in range(1,95):
	im2=Image.open("output"+str(scount)+".gif")
	scount=scount+1
		...
		for x in range(0,im2.size[1]):
			...
			if pix!=255 and pix!=10:
				...
				if checky==1:
					...
					working_slice.save("letter"+str(xcount)+".gif",quality=95)
					destinationDir='C:\\Users\\Prasanth\\Desktop\\comb\\Captcha-Cracker-master\\thecracked - Copy\\SBIcracked'
					shutil.move("letter"+str(xcount)+".gif", destinationDir)
					highy=999
					lowy=0
				xcord=x
				ycord=y
